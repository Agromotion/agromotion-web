...
export default function About() {
  return (
    <section id="about" className="section about-section" style={{ padding: '72px 40px', borderBottom: '1px solid var(--border-c)' }}>
      ...
      <h2 style={{ fontSize: 'clamp(28px,4vw,44px)', fontWeight: 600, lineHeight: 1.1, letterSpacing: '-.02em', marginBottom: 16 }}>Três pilares. Uma solução.</h2>
      <p style={{ fontSize: 16, lineHeight: 1.8, color: 'var(--text2)', maxWidth: 640, fontWeight: 300, marginBottom: 40 }}>
        Apesar de ser o mesmo ecossistema, o projeto divide-se em dois trabalhos finais independentes que se complementam — um focado no hardware e firmware, outro no software e cloud.
      </p>
      <div className="about-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {PILLARS.map((p) => (
          <div key={p.title} className="about-card" style={{ background: 'var(--surface)', border: '1px solid var(--border-c)', borderRadius: 12, padding: 28, gridColumn: p.wide ? '1 / -1' : undefined }}>
